...
export default class StreamSchedule extends StreamFilter {
    /**
    ...
    constructor(language) {
        ...
        this.schedule = JSON.parse(fs.readFileSync(new URL(`../data/stream-schedule.${language}.json`, import.meta.url), { encoding: 'utf8' }));
    }
    ...
    getNextScheduledShow() {
        ...
        for(const show of this.schedule) {
            ...
            if(
                (start.day === now.getUTCDay() || end.day === now.getUTCDay()) &&
                end.hour > now.getUTCHours() &&
                end.minute > now.getUTCMinutes() &&
                (!nextShow || StreamSchedule.compareDaytimes(nextShow.dstart, start) > EQUAL)
            ) {
                nextShow = show;
